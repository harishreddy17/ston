...
def generate_augmented_images_and_masks(data, img_size=256, start_mask_value=1, max_mask_value=25):
    ...
    for image_id, group in grouped:
        ...
        # Visualize the mask overlayed on the image with yellow color (BGR: [0, 255, 255] = Yellow)
        mask_overlay = augmented_image.copy()
        mask_overlay[augmented_mask_normalized > 0] = [0, 255, 255]  # Yellow color for mask overlay (BGR)

        # Save the overlay image
        cv2.imwrite("debug_mask_overlay.png", mask_overlay)  # Save the overlay image
        
        # Save the final augmented image and mask in separate folders
        base_name = image_id[0].split('/')[1].split('.')[0]
        
        # Define separate directories for lines and masks
        output_image_path = f"output_augmented_lines/{base_name}_lines_aug.png"
        output_mask_path = f"output_augmented_masks/{base_name}_mask_aug.png"
